...
export default function UploadStudio({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Basis-Metadaten für alle Requests
      const baseMeta = {
        post_type: postMode,
        tags: tags.join(','),
        make_public: String(makePublic),
        folder_id: targetFolderId,
        content_confirmed: 'true',
        title: postMode === 'einsatz' ? title : '',
        event_date: postMode === 'einsatz' ? eventDate : '',
        alarm_code: postMode === 'einsatz' ? alarmCode : '',
        location: postMode === 'einsatz' ? location : '',
        article_body: postMode === 'einsatz' ? articleBody : '',
      };
      ...
      // Jedes Bild einzeln -- max ~100 MB pro Request
      for (let i = 0; i < selectedImages.length; i++) {
        const img = selectedImages[i];
        if (img.source !== 'file' || !img.file) continue;
        ...
        fd.append('is_last', String(i === selectedImages.length - 1));
        fd.append('sort', String(i));
        if (postId) fd.append('post_id', postId);
        if (finalFolderId) fd.append('final_folder_id', finalFolderId);
        if (originFolderId) fd.append('origin_folder_id', originFolderId);

        // Metadaten nur beim ersten Bild
        if (i === 0) {
          Object.entries(baseMeta).forEach(([k, v]) => fd.append(k, v));
        } else {
          fd.append('make_public', String(makePublic));
        }

        fd.append('original_0', originalBlob, img.file.name);
        fd.append('preview_0', preview, `prev-${img.file.name}.jpg`);
        fd.append('download_0', download, `dl-${img.file.name}.jpg`);
        fd.append('caption_0', img.caption);

        setProgress(`Foto ${i + 1}/${selectedImages.length} wird hochgeladen …`);
        const res = await fetch('/api/intern/upload', { method: 'POST', body: fd });
        if (!res.ok) {
          const b = await res.json().catch(() => ({}));
          throw new Error(b.error ?? 'Upload fehlgeschlagen.');
        }
        const data = await res.json();
        if (i === 0) {
          postId = data.id;
          finalFolderId = data.final_folder_id ?? null;
          originFolderId = data.origin_folder_id ?? null;
        }
      }
