...
export default function UploadStudio({
  ...
  function handleSourceTab(tab: SourceTab) {
    setSourceTab(tab);
    if (tab === 'bibliothek' && libraryItems.length === 0) {
      loadLibrary('');
    }
  }
    ...
    try {
      ...
      for (let i = 0; i < selectedImages.length; i++) {
        ...
        if (img.source === 'file' && img.file) {
          ...
          formData.append(`original_${i}`, img.file);
              ...
              {(['neu', 'bibliothek', 'ordner'] as SourceTab[]).map((tab) => (
                <button
                  ...
                  className={`rounded px-3 py-1.5 text-[12px] font-semibold transition-colors ${
                    sourceTab === tab ? 'bg-white text-ink shadow-sm' : 'text-ink-2 hover:text-ink'
                  }`}
                >
                ...
                {libraryLoading ? (
                  ...
                  return (
                    ...
                    >
                      {item.file_preview ? (
                        <Image
                          src={directusAssetUrl(item.file_preview, 'width=120&quality=70')}
                          alt=""
                          fill
              ...
              {!openFolderId ? (
                ...
                <>
                  <div className="border-b border-line px-3 py-2">
                    <button
                      type="button"
                      onClick={() => { setOpenFolderId(null); setFolderPosts([]); }}
                          ...
                          return (
                            ...
                            >
                              {img.file_public_preview ? (
                                <Image
                                  src={directusAssetUrl(img.file_public_preview, 'width=120&quality=70')}
                                  alt=""
                                  fill
                                  className="object-cover"
                                />
        ...
        <div className="flex flex-col rounded-[10px] border border-line bg-white overflow-hidden">
          ...
          <div className="flex flex-1 flex-col gap-4 overflow-y-auto p-4">
            ...
            {postMode === 'einsatz' && (
              <div className="flex flex-col gap-3 rounded-md border border-line bg-panel p-3">
                ...
                <div>
                  ...
                  <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} className={fieldClass} placeholder="Zimmerbrand Hauptstraße" />
                </div>
                ...
                <div>
                  ...
                  <input type="text" value={location} onChange={(e) => setLocation(e.target.value)} className={fieldClass} placeholder="Musterstadt" />
                </div>
              </div>
            )}
            ...
            <div>
              ...
              <div
                className="flex flex-wrap gap-1.5 rounded-md border border-line-strong bg-white px-3 py-2 cursor-text min-h-[40px]"
