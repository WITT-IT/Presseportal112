...
async function uploadBuffer(
  ...
  if (!res.ok) {
    throw new Error(
      `Datei-Upload fehlgeschlagen (${res.status}): ${await res.text()}`
    );
  }
...
async function getSystemFolders(token: string, orgId: string) {
  ...
  if (!res.ok) {
    return { publicFolderId: null, privateFolderId: null, unsortedFolderId: null };
  }
  ...
  return {
    publicFolderId:
      rows.find((r) => r.system_role === 'public')?.id ??
      rows.find((r) => r.name === 'Öffentlich')?.id ??
      null,
    privateFolderId:
      rows.find((r) => r.system_role === 'private')?.id ??
      rows.find((r) => r.name === 'Privat')?.id ??
      null,
    unsortedFolderId:
      rows.find((r) => r.system_role === 'unsorted')?.id ??
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (imageIndex === 0) {
      ...
      const { publicFolderId, privateFolderId, unsortedFolderId } =
        await getSystemFolders(session.accessToken, user.organization.id);

      if (makePublic) {
        finalFolderId = publicFolderId ?? null;
        if (targetFolderIdRaw) {
          originFolderId = targetFolderIdRaw;
        }
      } else if (targetFolderIdRaw) {
        finalFolderId = targetFolderIdRaw;
      } else if (privateFolderId) {
        finalFolderId = privateFolderId;
      } else {
      ...
      if (originFolderId) {
        postBody.origin_folder_id = originFolderId;
      }
      ...
      if (!postRes.ok) {
        ...
        if (!postRes.ok) {
          throw new Error(
            `Beitrag anlegen fehlgeschlagen: ${await postRes.text()}`
          );
        }
      }
    } else {
      if (!existingPostId) {
        return NextResponse.json({ error: 'post_id fehlt.' }, { status: 400 });
      }
    ...
    if (originalFile && previewFile && downloadFile) {
      ...
      const uploadFolderId = finalFolderId ?? undefined;
      ...
      if (!imageRes.ok) {
        throw new Error(
          `Bilddatensatz anlegen fehlgeschlagen: ${await imageRes.text()}`
        );
      }
    }
